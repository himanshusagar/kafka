...
class Partition(val topicPartition: TopicPartition,
  ...
  def makeLeader(partitionState: LeaderAndIsrPartitionState,
    ...
    val (leaderHWIncremented, isNewLeader) = inWriteLock(leaderIsrUpdateLock) {
      ...
      info(s" stateChangeLogger Leader $topicPartition starts at leader epoch ${partitionState.leaderEpoch} from " +
  ...
  private def maybeIncrementLeaderHW(leaderLog: UnifiedLog, curTime: Long = time.milliseconds): Boolean = {
    ...
    leaderLog.maybeIncrementHighWatermark(newHighWatermark) match {
      case Some(oldHighWatermark) =>
        debug(s"hsagarWaterMark High watermark updated from $oldHighWatermark to $newHighWatermark")
        true
    ...
    try {
      ...
      case e: UnexpectedAppendOffsetException =>
        ...
        if (logEndOffset == log.logStartOffset &&
          ...
          info(s" hsagar Unexpected offset in append to $topicPartition. First offset ${e.firstOffset} is less than log start offset ${log.logStartOffset}." +
